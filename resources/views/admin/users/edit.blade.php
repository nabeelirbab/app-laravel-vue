...
@section('content')
    ...
    <div>
        <form action="{{ url()->current() }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="interests[]" id="interests" class="form-control" multiple="multiple">
                    @foreach ($genres as $interest)
                        <option {{ $user->interests->contains($interest->id) ? 'selected' : null }}
            ...
            <div class="form-group">
                ...
                <select name="genres[]" id="genres" class="form-control" multiple="multiple">
                    @foreach ($genres as $genre)
                        <option {{ $user->genres->contains($genre->id) ? 'selected' : null }} value="{{ $genre->id }}">
                            {{ $genre->name }}</option>
                    @endforeach
                </select>
            </div>
                ...
                <select name="role" required id="role" class="form-control">
                    ...
                    @foreach ($roles as $role)
                        <option value="{{ $role->id }}"
                            {{ $user->roles->contains('name', $role->name) ? 'selected' : null }}>
            ...
            <div class="form-group">
                <label for="password">Artist Type</label>
                @include('admin.partials.field-error', ['field' => 'artist_user_type'])
                <select name="artist_user_type_id" required id="artist_user_type" class="form-control">
                    <option>Select a Artist Type</option>
                    <option {{ $user->artist_user_type_id == 1 ? 'selected' : null }} value="1">Record label</option>
                        <option {{ $user->artist_user_type_id == 2 ? 'selected' : null }} value="2">Producer</option>
                </select>
            </div>
