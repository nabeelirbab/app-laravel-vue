...
## Hey {{ $user->first_name }}, we have finished verifying your profile! {{ (($user->roles->count() > 0) && ( in_array($user->roles->first()->name, ['artist_pro', 'pro'])) ) ? ' - Welcome to the PRO club!' : '' }}

<ul>
    @if($user->roles->count() > 0)
    <li>User type: {!! ( in_array($user->roles->first()->name, ['artist_pro', 'pro'])) ? 'Artist <b>PRO</b>' : \Illuminate\Support\Str::title($user->roles->first()->name) !!} </li>
    @endif
...
@if($user->roles->count() > 0)
    ...
    @elseif ( in_array($user->roles->first()->name, ['artist_pro', 'pro']) )
