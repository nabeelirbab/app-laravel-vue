	...
	<div>
		...
		<form @submit.prevent="handleSubmit" id="account-form">
			...
			<div class="flex flex-row justify-between" id="in">
				<input :style="style" v-model="inputs[idx]" :class="inputClass" @keyup="inputFocus" @focus="check($event)"
					@change="handleChange(idx, $event)" v-for="(inp, idx) in inputCount" :key="idx" :tabindex="idx"
...
export default {
	...
	methods: {
		async handleSubmit() {
			...
			const nonIntValues = this.inputs.filter(value => !Number.isInteger(Number(value)));
