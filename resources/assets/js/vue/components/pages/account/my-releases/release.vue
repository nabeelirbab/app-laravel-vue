<template>
    <div style="margin-bottom: 20px;">
            ...
            <div class="release-info flex">
                ...
                <div class="release-details">
                    <div class="flex justify-between">
                        ...
                        <div>
                            <span class="text-sm"><span class="text-blue">Release Date: </span> {{
                                formattedDate
                            }}</span>
                        </div>
                        <div v-click-outside="() => contextMenu = false">
                            ...
                            <div v-show="contextMenu" style="position: relative;">
                                <ul class="context-menu">
                                    ...
                                    <li v-if="release.status === 'live'"
                                        @click="updateStatus({ release: release, status: 'offline' })">Take Offline</li>
                                    <li v-if="release.status === 'offline'"
                                        @click="updateStatus({ release: release, status: 'live' })">Make Live</li>
                    ...
                    <div class="release-description flex text-xs">
                        <div style="width: 85%;">
                            <p><span class="text-blue">{{ release.class | capitalize }} description: </span> {{
                                release.description
                            }}</p>
                        </div>
                    ...
                    <div class="text-xs text-blue tab-buttons">
                        <span @click="changeTab('details')" class="tabButton">
                            {{ release.class | capitalize }} details
                        </span>
                        <span @click="changeTab('tracks')"
        ...
        <div class="tabs text-sm" v-if="tabOpen">
            ...
            <track-list v-if="tab === 'tracks'" :tracks="release.tracks" />
        </div>
    </div>
</template>
